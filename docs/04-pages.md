...
- `general/General.vue` — общая страница раздела с выбором региона и сезона; `rankDistribution/` — столбчатое распределение игроков по рангам и рейтингу с одиночным и множественным выбором. Под графиком находятся глобальные таблицы техники и карт с общим календарным фильтром дней в поповере; выбранные столбики графика фильтруют обе таблицы по рангу. Общий `shared/daySelector/DaySelector.vue` поддерживает произвольный множественный выбор и непрерывный интервал.
