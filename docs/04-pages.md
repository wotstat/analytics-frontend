...
- `general/General.vue` — общая страница раздела с выбором региона и сезона; `rankDistribution/` — столбчатое распределение игроков по рангам и рейтингу с одиночным и множественным выбором. Под графиком находятся глобальные таблицы техники и карт с общим календарным фильтром дней в поповере; выбранные столбики графика фильтруют обе таблицы по рангу. Таблица техники умеет группировать строки по навыку Натиска, а без группировки показывает самый популярный навык и распределение навыков в тултипе. Общий `shared/daySelector/DaySelector.vue` поддерживает произвольный множественный выбор и непрерывный интервал.
