...
`Vehicles.vue` (`/session/vehicles`) пока содержит заготовку таблицы на `div` и отдельную секцию
...
Состояние хранится на странице, загрузка статистики и сохранение в URL пока не подключены.
...
сюда не входят: это будущие локальные фильтры таблицы после запроса.
...
Для будущего WHERE по `VehiclesStatistics`:
...
режимам из префикса ключа (`schema-pk-filter-on-orderby`). Сами запросы ещё не подключены.
