...
- **`axis/`**, **`ticks/`** (`TicksByLabels` — composite по уровням, `TicksByValues`, `BaseOffsetTicks`), **`labels/autoLabels/`** (автогенерация подписей: `steppedGenerator`, `arrayGenerator`).
  ...
  - Длина штрихов (`start`/`end`) у `TicksByValues` задаётся только в конструкторе. У `TicksByLabels` она настраивается на лету (`updateOptions`) и по умолчанию берётся из итога кадра: у каждого уровня есть `suggestedStart` — `Infinity` у `interval` (отсюда штрихи сквозь зону подписей, недельные разделители лидерборда), `4` у `classic`/`classic-flow`/`cell` на уровне подписей, `0` у остальных уровней. Явный `start` перебивает подсказку.
...
Подписи и тики связаны одним значением — **уровнями тиков кадра**. `BaseLabels.calculateLabelsFrame()` возвращает `LabelsFrame = { labels, tickLevels }`, а `render()` атомарно публикует уровни наружу (`getTickLevels()`). Пробный `getSize()` их не трогает: туда не должны попадать промежуточные примерки лейаута. Пустые bounds или отсутствие победителя дают пустой список — старые значения не остаются. Наружу отдаётся **только то, что читает рендерер тиков**: ни победившего кандидата, ни стратегии там нет — диагностику стенд добывает своими средствами, в контракт движка она не просачивается.
...
- **Дедупликация — по порядку списка:** кто выше, тот забирает общее значение. Отсюда отсутствие двойной линии в полночь начала недели; порядок настраиваемый, «подписи всегда сильнее» больше не зашито.

**Наследование настроек кандидата** (`LabelStepOverrides`): `ticks === undefined` — берётся общий `options.ticks`, свой список заменяет общий целиком (merge по индексам нет). `steppedOverrides` дописывает в хвост 10 удвоений последнего шага **начиная с ×2**, и хвост наследует все нормализованные настройки последнего явного кандидата — формат, стратегию, `ticks`; меняется только шаг.
...
`.label-ticks` движок добавляет сам тому уровню, который стоит по подписям, — это факт «под этими тиками есть текст», а не порядковый номер. Отсюда правило для стилей страницы: **цвет единицы** задавать через `.day-ticks` / `.week-ticks` / `.hour-ticks`, а **силу порядка** — через `.tick-level-N`. Тогда день выглядит днём независимо от того, на каком он сейчас уровне.
