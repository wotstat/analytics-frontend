...
- **`FloatingTooltip.vue`** — плавающий тултип поверх страницы. Телепортируется в `body` и отдаёт позиционирование `PopoverStyled` (`shared/uiKit/popover`), поэтому бесплатно получает флип у краёв экрана, стрелку, карточку и анимацию. Целью выступает `VirtualElement` — прямоугольник считается из абсолютных координат `ctx` минус текущий скролл, без промежуточного элемента в DOM, поэтому тултип остаётся приклеенным к точке и при скролле страницы без перерисовки графика. Проп `anchor`: `pivot` (точка, по умолчанию), `cursor`, `pivot-x` / `pivot-y` (якорь-линия во всю высоту/ширину графика — тултип встаёт сбоку). Дальше пробрасываются `placement`, `offset`, `viewportOffset`, `arrowSize`, `class`; по умолчанию тултип не перехватывает указатель (`interactive`). Во время анимации скрытия контент остаётся от последнего непустого `ctx`, поэтому слот всегда получает непустой `ctx`.
