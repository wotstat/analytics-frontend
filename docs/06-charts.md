...
- **Автомасштабирование** — ядро сначала находит границы данных внутри заданного диапазона осей, затем применяет `renderBoundsDataPadding` в единицах данных. `renderBoundsPxPadding` измеряет область построения для этих желаемых границ в текущем кадре и за один проход переводит отступ в CSS-пикселях в единицы данных через `ChartSpace`. Если размер подписей после расширения границ меняет длину той же оси, используй отступ в данных: повторного подбора размеров нет. `renderBoundsDomain` ограничивает только автоматически вычисляемые края (например, `minY: 0`); явно заданные через `setRenderBounds` границы остаются приоритетными. Если ограничение сделало бы диапазон пустым, оно не применяется к этой оси.
...
- **`axis/`**, **`ticks/`** (`TicksByLabels` — composite по уровням, `TicksByValues`, `BaseOffsetTicks`), **`labels/autoLabels/`** (автогенерация подписей: декларативный `ValueSource` и фабрика кандидатов `labelCandidates`).
  ...
  - `AutoLabels` перебирает кандидаты из `values` от мелкого к крупному и берёт первый, на котором подписи не пересекаются. `classesForValue` задаёт CSS-классы отдельных подписей. Вторым аргументом в `labelForValue` приходит объект `LabelContext` с `candidateIndex`; у результата `labelCandidates({ values })` контекст дополнительно содержит `valueIndex` исходного массива. В итоге кадра этих индексов нет намеренно, там лежит только то, что читают тики. `keyForValue` по умолчанию равен **тексту** подписи, а ключ — это тождество `<text>` между кадрами: повторяющийся текст в пределах одного этажа (часы по дням, дни недели) схлопывает разные значения в один элемент, и часть подписей молча пропадает при живых тиках. Для `{ values }` фабрика по умолчанию ставит ключ по числовому значению; в остальных случаях для повторяющихся форматов задавай `keyForValue` явно — движок про коллизию не предупредит. Как позиционировать подпись, решает `strategy`:
...
в одной полосе, от сильного к слабому. Каждый источник принимает обычный `ValueSource`
или объект `LabelOptions & { source: ValueSource }` со своими форматтером, ключом,
классами, `from`/`to`, `padding`, `onlyFitted` и `ticks`. Настройки наследуются от этажа,
а этаж — от корневых `Options`. Обычный `source` сохраняет прежний алгоритм.

`strategy` задаётся отдельно от `LabelOptions`, только на этаже или в корневых настройках.
Внутри `priorities` стратегия общая: `classic` или `classic-flow`. `cell` и `interval`
доступны только обычным источникам, в том числе на отдельных этажах.
Неподдерживаемая стратегия у приоритетного этажа или любая стратегия у вложенного
источника отклоняется при создании и обновлении настроек.

```ts
values: [{
  priorities: [
    { source: versionGenerator, classes: 'version', padding: 12 },
    { source: patchGenerator, classes: 'patch', padding: 8 },
    { source: micropatchGenerator, classes: 'micropatch', padding: 4, ticks: [] },
  ],
  maxLabelSize: 240,
...
Сначала отбирается старший приоритет. Следующий исключает подписи, пересекающиеся
с уже принятыми старшими, и выполняет собственный локальный отбор. Внутри приоритета
побеждает стабильный хеш ключа и значения (при равенстве — меньшее значение).
Более сильный сосед блокирует подпись, даже если сам проиграл другому соседу:
цепочка решений не продолжается за пределы непосредственного окружения. Поэтому
в плотных цепочках допустимы свободные места — максимальное заполнение не является целью.
Ключ по умолчанию равен значению, ключи изолированы индексом приоритета.
Чтобы переключение видимости не меняло этот индекс, оставляй пустые источники на месте.

`maxLabelSize` — обязательная верхняя граница размера любого текста вдоль оси в пикселях,
включая тексты за экраном. Она ограничивает радиус конфликта
`R = maxLabelSize + max(priorities.padding.clip)`.
Каждый старший приоритет рассчитывается на два радиуса дальше следующего: для его
соседей и их старших блокировщиков. Генератор читается только в этом конечном диапазоне;
он должен выдавать конечные числа по возрастанию и уметь начинать с `startFrom`.
Повторы значений пропускаются. Превышение размера или бюджета 10 000 значений на источник
завершает расчёт ошибкой, а не обрезает зависимости молча. Встроенный `{ step }` здесь
ограничивается этим расчётом, без обычного лимита генератора в 1000 значений.

Зазор при проверке пересечения равен максимуму `padding.clip` двух подписей.
Отбор считается по исходным координатам данных. При неизменных масштабе, источниках,
ключах и метриках текста панорамирование его не меняет. Затем применяется обычный `flow`;
для соседней пары используется максимум их `padding.flow`. В диапазон расчёта также
входят соседи для сдвигов у края. `onlyFitted` применяется после отбора отдельно к каждому
источнику: скрытая им подпись продолжает блокировать младшие приоритеты.
Для частичного ухода текста
за край нужны `onlyFitted: false`, клип слота и разрешённая видимость `.value-outside-bounds`.
Зум может менять отбор. Приоритетный этаж всегда прореживается локально и сам не вызывает
переход к следующему кандидату шага.

У каждого приоритета свой список тиков, по умолчанию унаследованный от этажа.
`ticks: 'labels'` следует только за отобранными подписями этого источника до `onlyFitted`;
`ticks: []` отключает его тики. Свой список полностью заменяет унаследованный.
Явные источники тиков могут сохранять тики всех событий, как у аннотаций версий.
Уровни тиков объединяются по порядку приоритетов, затем этажей.
Стенд `/debug/chart/axis#priority-labels` позволяет менять отступы, тики и `onlyFitted`
младшего источника, общую стратегию и панорамировать при фиксированном масштабе.
...
type TickSource =
  ...
  | ValueSource
  | { source: ValueSource | 'labels', minPixelSpacing?: number, from?: number, to?: number, classes?: Classes }

type ValueSource =
  | ValueGenerator                              // произвольный источник, например календарный
  | { step: number, offset?: number }           // равномерный шаг
  | { values: readonly number[] }               // отсортированный явный набор

ticks?: TickSource | readonly TickSource[]
```

Источник значений задаётся декларативным `ValueSource`: `{ step, offset? }`, `{ values }` или собственным `ValueGenerator`. Для тиков также есть шорткат `'labels'`, чтобы не дублировать источник подписей. Явный массив должен быть отсортирован по возрастанию. Отдельный уровень получается на каждый элемент списка, `classes` — из его же настроек. Скрытый уровень остаётся в списке с пустым `values`: иначе порядковые номера (а с ними и классы) поехали бы на следующем кадре.
...
**Наследование настроек уровней:** `ticks === undefined` у первого этажа кандидата — берётся общий `options.ticks`, свой список заменяет общий целиком (merge по индексам нет); следующие этажи по умолчанию получают тики по собственным подписям. `labelCandidates({ step })` дописывает в хвост 10 удвоений последнего шага **начиная с ×2**, и хвост наследует все нормализованные настройки последнего явного уровня — формат, стратегию, `ticks` и `classes`; меняется только шаг. В `labelCandidates({ values })` плоский массив создаёт один кандидат, а массив массивов или объектов — последовательность кандидатов без автоматического fallback. Форматтер получает `candidateIndex` текущего кандидата и `valueIndex` внутри его массива.
...
values: labelCandidates({ step: [
  { step: DAY, labelForValue: v => `${1 + v / DAY} день`, ticks: [{ source: 'labels', classes: 'day-ticks' }, hourTicks] },
  { step: WEEK, labelForValue: v => `${1 + v / WEEK} неделя`, ticks: [{ source: 'labels', classes: 'week-ticks' }, dayTicks, hourTicks] },
]})
...
У source две разные идентичности. `interaction.id: symbol` уникален для конкретного экземпляра и участвует в локальной identity хитов. `interaction.tag?: InteractionTag` — опциональная семантическая метка всего плота, которую намеренно повторяют у соответствующих source разных графиков. Все plot-конструкторы прокидывают её одинаковой опцией `interactionTag`; одинаковый tag может принадлежать нескольким source даже внутри одного графика. Если плот содержит несколько семантических серий, source может сопоставлять несколько tags своим targets: так Bar использует `dataset.interactionTag`. Для follower-подсветки `Highlight` всегда вызывает единый `source.getTargets(tag)` и не знает, относится tag ко всему плоту или его датасету.
...
- **`MarkerOverlay`** (`components/markerOverlay/`) — один SVG-маркер на уникальный anchor selection, `classesForHit(hit)` задаёт стилизацию вызывающий, классы исходного плота не копируются автоматически. Чтобы цвет совпадал с динамической серией, вызывающий может назначить линии и маркеру общий класс и правило `color`, а сам видимый круг красить через `.interaction … { fill: currentColor }`. Чёрный круг-вырез находится внутри переданного `mask`, поэтому селектор заливки должен учитывать родителя и не перекрашивать маску.
