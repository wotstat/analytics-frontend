...
| `recordBoxCount` | `UInt16` | Количество контейнеров, представленных одной строкой события (у старых записей — 1) |
...
24 сентября 2026 года `GameVersions` дополнена 129 историческими записями для RU/EU/NA/ASIA/CN
из `Arenas` (май 2024 — июнь 2025). Изначально их `datetime` показывал время первого доступного
снимка игровых ресурсов. Четыре старые сборки, повторно загруженные 9 октября 2024 года,
исключены из-за неверной даты.

Затем `datetime` первой точной сборки 85 патчей перенесён на начало дня первого боя
этого патча в `Event_OnBattleStart` (UTC). Перенос сделан только там, где первая сохранённая
сборка патча имеет микропатч `.0`: регион и номер патча связывают событие с этой сборкой,
а её полный номер и хеш остаются прежними. У 10 патчей первая точная сборка уже имеет
микропатч `.1` или `.2`; её время не менялось. Из прежних 406 строк ещё 321 сохранила исходное время.
Например, первая сборка RU 1.37
`v.1.37.0.0 #2005` перенесена с 5 на 3 сентября 2025 года.

Для недостающей истории до версии 1.28 добавлены 25 строк с условным хешем `#0000` и номером
микропатча `.0`. Они представляют только дату первого боя версии/патча из `Event_OnBattleStart`,
не подтверждают существование сборки с таким хешем; `modsFolderName` у них пустой.
[Список добавленных строк](game-version-patch-history-2026-09-24.csv) содержит исходное событие
и число боёв в первый наблюдаемый день. Существовавшие 406 точных строк не изменялись.
В аннотациях и селекторе строки `#0000` показываются на уровнях версии и патча, но скрыты
на уровне микропатча. История событий начинается в январе 2024 года: более ранние даты
по этому источнику определить нельзя.
