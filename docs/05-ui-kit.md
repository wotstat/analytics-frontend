...
- **`tooltip/`** — тултипы на директивах. Глобальная `v-tooltip` показывает текст; объект значения поддерживает `text`, параметры поповера и опциональный `target: HTMLElement`. Для произвольного содержимого есть `defineTooltip`, а `useTooltip(Component, options)` сразу создаёт локальную директиву, передающую значение биндинга в props компонента. Опция `valueAdapter` позволяет отдельно преобразовать значение в props содержимого и параметры тултипа. На элементе с директивой отслеживаются наведение указателя и касания (фокус сам по себе тултип не открывает), а тултип позиционируется относительно `target` (по умолчанию — относительно самого элемента).
