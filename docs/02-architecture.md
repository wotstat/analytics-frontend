...
- **`pages/debug/shared/`** — служебная обвязка: `DebugPage.vue` (заголовок + описание + путь к источнику), `DebugSection.vue` (секция с якорем и строкой «на что смотреть»), `debug.scss` (не-scoped, всё внутри `.debug-root`: классы контролов, сцен `.debug-stage` с модификаторами, лога событий, таблицы состояний), `fixtures/` (синтетика с сидом и реальные запросы **в одной форме данных**, чтобы переключатель источника стоил пары строк).

Конвенции страниц: контролы — нативные `input`/`select`/`button` на классах из `debug.scss`; у каждой секции есть описание «что должно произойти» и путь к файлу-источнику; тач-специфика подписывается словами в `.debug-note`; ценнее всего тумблеры, которые **ломают компонент нарочно** (снос цели из DOM, вырожденные bounds, забытый `dataDidUpdate`) — именно они показывают границы поведения. Поповеры и меню телепортируются в `#popover-root`, тултипы директив — в свой `#tooltip-root`, то есть наружу `.debug-root`, и классы `.debug-*` до их содержимого не достают.
