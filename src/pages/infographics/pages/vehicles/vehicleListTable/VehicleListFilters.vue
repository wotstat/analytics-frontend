<template>
  <button ref="trigger" class="filter-trigger" :class="{ active: activeCount > 0 }" @click="open = !open">
  ...
  <PopoverAutoClose v-model="open" :target="trigger" :placement="['bottom-start', 'bottom-float']"
    ...
    <div class="vehicle-list-filters">
      <div class="title">
        ...
        <button v-if="activeCount" class="reset"
...
import {
  createLocalVehicleFilters, DEFAULT_MIN_BATTLES, DEFAULT_MIN_PLAYERS,
...
const activeCount = computed(() => {
  ...
  if (showVehicleFilters) count += levels.length + nations.length + types.length + Number(onlyActual)
