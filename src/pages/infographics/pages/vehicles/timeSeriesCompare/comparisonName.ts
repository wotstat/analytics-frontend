  ...
  for (const key of different) {
    switch (key) {
      ...
      case 'team':
        ...
        break
      case 'platoon':
        labels.push({ any: 'Любой взвод', solo: 'Без взвода', duo: 'Взвод: 2 игрока', trio: 'Взвод: 3 игрока', large: 'Взвод: 4 и более' }[filters.platoon])
        break
      case 'result':
        labels.push({ any: 'Любой результат', win: 'Победа', loss: 'Поражение', draw: 'Ничья' }[filters.result])
        break
      case 'battleLevel':
