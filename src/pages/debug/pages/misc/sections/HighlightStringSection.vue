  ...
  <DebugSection title="HighlightString" id="highlight-string" source="src/shared/uiKit/highlightString/"
    ...
    <div class="debug-col">
      ...
      <p class="debug-note" v-if="forceBroken">
        ...
        подсунуть highlight: undefined мимо системы типов (как здесь), в шаблон попадает объект вместо строки. Это не
        «[object Object]»: Vue-интерполяция сериализует объект через <span class="debug-value">JSON.stringify(val,
        null, 2)</span> — с переносами и отступами, а браузер в обычном тексте схлопывает их в пробелы, так что на
        экране выше — однострочное <span class="debug-value">{ "text": "Пример строки" }</span>.
      </p>
    </div>
  </DebugSection>
</template>
