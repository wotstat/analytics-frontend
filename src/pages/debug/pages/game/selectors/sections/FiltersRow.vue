<template>
  <DebugSection title="Все четыре в одном ряду" id="filters-row"
    ...
    <p class="debug-note">
      Ровно та же табличка отвечает на вопрос «одинаковой ли они высоты»: нет. Пустая строка бейджей — 24px (высота
      кнопки «выбрать»), <span class="debug-value">GameSelector</span> — 40px, и с каждым переносом бейджей строка
      подрастает ещё на ~28px. Поэтому в бою ячейки выровнены по
