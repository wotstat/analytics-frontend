<template>
  <DebugSection title="minimapUrl: как собирается адрес картинки" id="minimap-url"
    description="STATIC_URL + префикс игры + tag + формат. Если gameplay задан и у арены нашлась мета с '_comp7' в имени minimap-файла — URL уходит в отдельную папку comp7/. tagToImageName в этой сборке НЕ участвует (см. заметку ниже) — он показан отдельно, как его использует, например, ArenaSelectorContent."
    ...
    <p class="debug-note">
      ...
      вида <span class="debug-value">01_karelia</span>) это тождественная операция. minimapUrl строит URL сам, своей
      строкой <span class="debug-value">`${STATIC_URL}/${gamePrefix}/latest/arenas/minimap/${tag}.${format}`</span>,
      и tagToImageName не вызывает вовсе — только сам режет опциональный префикс <span
        class="debug-value">spaces/</span>.
...
const url = computed(() => minimapUrl(rawTag.value, game.value, gameplay.value, format.value))
</script>
