<template>
  <DebugSection title="Уровни тиков: недели → дни → часы" id="tick-levels"
    ...
    <p class="debug-note">
      ...
      места. Порог считается по <b>полному сгенерированному</b> набору до дедупликации в рендерере: иначе удаление
      каждой седьмой линии на границе недели делало бы уровень «разреженнее», чем он есть. Уровню подписей порог тоже
      можно задать — настройки у всех уровней одни и те же, — но смысла в этом мало: его плотность уже проверил
      перебор кандидатов.
    </p>

    <p class="debug-note">
      <b>Дедупликация происходит перед отрисовкой:</b> расчёт сохраняет полные наборы значений всех уровней, а общую
      линию отдаёт уровню с самым длинным фактическим <span class="debug-value">start</span>. Поэтому interval-тик на
      границе недели проходит до более дальнего этажа, даже если такой же день или час встречается раньше в списке.
      При равной длине побеждает более ранний уровень. В таблице разница между «значений в расчёте» и «линий в DOM»
      показывает как дедупликацию, так и отсечение линий за видимыми краями.
    </p>
    ...
    <p class="debug-note">
      ...
      кадра. Пробный <span class="debug-value">getSize()</span> может рассчитать подписи для размера слота, но не
      публикует их уровни тиков — иначе наружу попадали бы промежуточные примерки лейаута.
    </p>
