<template>
  <DebugSection title="Типы строк: Button, Header, Separator, Child" id="line-types"
    ...
    <p class="debug-note">
      ...
      всё равно рендерится узкой полоской (и, поскольку ширина считается по элементу, оказывается в левом верхнем
      углу). Проверять «есть ли что показывать» должен вызывающий код. У «пустых подписей» видно, что
