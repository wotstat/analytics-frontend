...
export type TouchZoomAxis = 'x' | 'y'
export type AxisBounds = { min: number, max: number }
export type LayoutValue = { x: number, y: number, width: number, height: number }

export const TOUCH_ZOOM_MIN_AXIS_DISTANCE = 48
export const TOUCH_ZOOM_ACTIVATE_AXIS_DISTANCE = 72
export const TOUCH_ZOOM_MIN_DISTANCE = 8
...
export const SPRING_EPSILON_PX = 0.1
export const SPRING_VELOCITY_EPSILON_PX = 1
...
export function getAxisPosition(axis: TouchZoomAxis, point: Point, layout: LayoutValue): number {
  if (axis === 'x') return (point.x - layout.x) / layout.width
  return 1 - (point.y - layout.y) / layout.height
}

export function getAxisSize(axis: TouchZoomAxis, layout: LayoutValue): number {
  return axis === 'x' ? layout.width : layout.height
}

export function getTouchDistance(first: Point, second: Point): number {
...
export function getAxisBounds(bounds: Bounds, axis: TouchZoomAxis): AxisBounds {
  return axis === 'x' ? { min: bounds.minX, max: bounds.maxX } : { min: bounds.minY, max: bounds.maxY }
}

export function setAxisBounds(bounds: Bounds, axis: TouchZoomAxis, value: AxisBounds): void {
  if (axis === 'x') {
